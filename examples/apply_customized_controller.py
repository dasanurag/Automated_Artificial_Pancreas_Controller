...
path = './results/combined_adult_model'
# try:
#     os.makedirs(path)
# except OSError as e:
#     if e.errno != errno.EEXIST:
#         raise
tr_model = ACKTR.load("ACKTR_MlpLSTM_adult_def_reward")
percent_time = []
now = datetime.now()
...
for i,p in enumerate(person_options):
    ...
    # patient = T1DPatient.withName(p)
    # register(id='simglucose-'+patient_id+'-v0',entry_point='simglucose.envs:T1DSimEnv',kwargs={'patient_name': p})
    # env = gym.make('simglucose-'+patient_id+'-v0')

    print(env.reset())
    controller = MyController(0, model = tr_model) #BBController() #
